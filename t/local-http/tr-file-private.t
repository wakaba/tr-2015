...
test {
  ...
  return Promise->resolve->then (sub {
    return Promise->resolve->then (sub {
      my @p;
      my $eurl = percent_encode_c q<file:///private/notfound/> . rand;
      for my $t (
...
test {
  ...
  return Promise->all ([
    ...
    my $account2 = $_[0]->[1];
    my $key = rand;
    my $eurl = percent_encode_c q<file:///private/> . $key;
    (git_repo $c->received_data->{repos_path}->child ("private/$key"), files => {
      hoge => 'foo',
    })->then (sub {
